<script>
  ...
  function DrawingBoard(opts) {
      ...
      // setup the canvas
      this.margin = 8;
      this.ctx = this.canvas.getContext('2d', { willReadFrequently: true, imageSmoothingQuality: 'high' });
      ...
      this.render = (e) => {
          requestAnimationFrame(() => {
            ...
            if (e.touches && e.touches[0]) {
              this.ctx.lineTo(e.touches[0].clientX - this.canvasOffsetX, e.touches[0].clientY - this.canvasOffsetY);
            }
            else {
              this.ctx.lineTo(e.clientX - this.canvasOffsetX, e.clientY - this.canvasOffsetY);
            }
            this.ctx.stroke();
          });
      }
...
{#each $gameState.player as player, index}
    <div id="{player}" class="drawing-board"></div>
    {
        new DrawingBoard( {
